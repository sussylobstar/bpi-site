...
export default function Partner() {
  return (
    <section id="dealers" className="bg-ink-2 text-white">
      <div className="grid lg:grid-cols-2">
        ...
        <div className="gutter py-[clamp(64px,9vw,132px)] min-w-0">
          <div className="max-w-[560px]">
            <h2 className="font-extralight text-[clamp(30px,4vw,58px)] leading-[1.06] tracking-[-0.01em] text-balance">
              Built to make you the easiest floor to buy.
            </h2>
            <p className="mt-6 text-[clamp(15px,1.15vw,17px)] leading-[1.7] text-white/70 font-light max-w-[48ch]">
            ...
            <dl className="mt-[clamp(36px,5vw,56px)] grid grid-cols-2 gap-px bg-white/12 border border-white/12">
              {DEALER_VALUE.map((v) => (
                <div key={v.label} className="bg-ink-2 p-[clamp(18px,2vw,28px)]">
                  ...
                  <dd className="mt-2.5">
                    ...
                    <span className="mt-1 block text-[12.5px] leading-relaxed text-white/55 font-light">
                      {v.desc}
                    </span>
                  </dd>
                </div>
            ...
            <div className="mt-[clamp(32px,4vw,48px)] flex flex-wrap items-center gap-4">
              <Link
                to="/become-a-dealer"
                className="inline-flex items-center gap-2.5 bg-red px-7 py-[15px] text-[15px] font-normal hover:bg-red-deep transition-colors"
              >
                Become a Dealer <span aria-hidden>→</span>
              </Link>
              <Link
                to="/contact"
                className="inline-flex items-center gap-2.5 px-2 py-[15px] text-[15px] font-light text-white/80 hover:text-white transition-colors"
              >
                Talk to a rep
              </Link>
            </div>
