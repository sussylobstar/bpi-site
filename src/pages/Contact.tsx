    ...
    <>
      ...
      <Section tone="ink">
        <div className="grid gap-x-16 gap-y-12 lg:grid-cols-[1.2fr_1fr]">
          <div>
            <h2 className="font-extrabold text-[clamp(22px,2.4vw,32px)] tracking-[-0.01em] mb-8">
              Send us a message
            </h2>
            <DemoForm
              fields={FIELDS}
          ...
          <aside className="lg:pl-8 lg:border-l border-ink/10">
            <h2 className="text-[12px] tracking-[0.2em] uppercase text-ink/50">
              World Headquarters
            </h2>
            <address className="mt-4 not-italic text-[15px] leading-relaxed text-ink/65 font-normal">
              {hq.address}
              <br />
              {hq.city}, {hq.state} {hq.zip_code}
            </address>
            <dl className="mt-6 space-y-3 text-[15px] font-normal">
              <div>
                <dt className="text-[12px] tracking-[0.16em] uppercase text-ink/45">Phone</dt>
                <dd>
                  <a href={CONTACT.phoneHref} className="text-ink/75 hover:text-red transition-colors">
                    {CONTACT.phone}
                  </a>
                </dd>
              </div>
              <div>
                <dt className="text-[12px] tracking-[0.16em] uppercase text-ink/45">Email</dt>
                <dd>
                  <a href={`mailto:${CONTACT.email}`} className="text-ink/75 hover:text-red transition-colors">
                    {CONTACT.email}
                  </a>
                </dd>
              </div>
            </dl>
            <p className="mt-8 text-[13px] leading-relaxed text-ink/45 font-normal max-w-[36ch]">
