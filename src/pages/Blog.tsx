        ...
        <ul className="border-t border-white/10">
          {POSTS.map((p) => (
            <li key={p.id} className="border-b border-white/10">
              ...
              >
                <p className="text-[12px] tracking-[0.14em] uppercase text-white/45">
                  {p.category}
                  <span className="block mt-1 text-white/35 tracking-normal normal-case">{p.date}</span>
                </p>
                <div>
                  <h2 className="font-light text-[clamp(21px,2.4vw,32px)] leading-snug tracking-[-0.01em] group-hover:text-white text-white/90">
                    {p.title}
                  </h2>
                  <p className="mt-3 text-[15px] leading-relaxed text-white/60 font-light max-w-[62ch]">
                    {p.excerpt}
                  </p>
                </div>
